...
namespace d60.Cirqus.Tests.Diagnostics
{
	...
	public class TestProfiler : FixtureBase
	{
		...
		public class AnotherRoot
			: AggregateRoot,
			  IEmit<SomethingHappenedInAnotherRoot>
		{
			public void DoStuff()
			{
		...
		private class Profilero : IProfiler
		{
			...
			public void RecordAggregateRootGet(
				TimeSpan elapsed,
			...
			public void RecordAggregateRootExists(
				TimeSpan elapsed,
				string aggregateRootId)
			{
			}

			public void RecordEventBatchSave(
				TimeSpan elapsed,
				Guid batchId)
			{
			}
		...
		private class SlowWrapper : IEventStore
		{
			...
			public IEnumerable<EventData> Load(
				...
				long firstSeq = 0)
			{
				foreach (var e in _innerEventStore.Load(
					         aggregateRootId,
					         firstSeq
				         ))
				{

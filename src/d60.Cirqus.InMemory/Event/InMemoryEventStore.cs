...
namespace d60.Cirqus.InMemory.Events;

public class InMemoryEventStore : IEventStore, IEnumerable<EventData>
{
	readonly HashSet<string> _idAndSeqNoTuples = new();
	readonly List<EventBatch> _savedEventBatches = new();
	...
	public long GetLastGlobalSequenceNumber()
	{
		lock (_lock)
		{
			var events = _savedEventBatches.SelectMany(e => e.Events);
			return events.Any() ? events.Max(e => e.GetGlobalSequenceNumber()) : -1;
		}
	}

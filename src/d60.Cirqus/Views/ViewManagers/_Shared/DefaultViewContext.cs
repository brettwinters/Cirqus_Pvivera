...
public class DefaultViewContext : IViewContext, IUnitOfWork
{
	...
	TAggregateRoot GetFromCacheOrNull<TAggregateRoot>(string aggregateRootId, long globalSequenceNumber) where TAggregateRoot : class
	{
		...
		if (!_cachedRoots.TryGetValue(aggregateRootId, out entry))
		{
			return null;
		}
	...
	public TAggregateRoot Load<TAggregateRoot>(string aggregateRootId) where TAggregateRoot : class
	{
		if (CurrentEvent == null)
		{
			throw new InvalidOperationException(
				$"Attempted to load aggregate root {typeof(TAggregateRoot)} with ID {aggregateRootId} in snapshot at the time of the current event, but there was no current event on the context!"
			);
		}
	...
	class FrozenAggregateRootService : IUnitOfWork
	{
		...
		{
			throw new InvalidOperationException(
				$"Aggregate root {aggregateRootType} with ID {_aggregateRootInfo.Id} attempted to emit event {e}, but that cannot be done when the root instance is frozen! (global sequence number: {_aggregateRootInfo.GlobalSequenceNumberCutoff})"
			);
		}

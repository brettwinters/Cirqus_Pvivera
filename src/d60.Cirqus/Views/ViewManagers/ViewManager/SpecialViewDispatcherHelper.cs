...
/// </summary>
public class SpecialViewDispatcherHelper<TViewInstance> where TViewInstance : ISubscribeTo, IViewInstance, new()
{
	...
	/// </summary>
	public SpecialViewDispatcherHelper()
	{
	...
	/// <summary>
	/// Dispatches the given domain event to the view instance.
	/// The view instance's <see cref="IViewInstance.LastGlobalSequenceNumber"/> will automatically be
	/// updated after successful dispatch. If the global sequence number of the event is lower or equal
	/// to the view instance's <see cref="IViewInstance.LastGlobalSequenceNumber"/>, the event is ignored.
	/// </summary>
	public void DispatchToView(
		...
		TViewInstance view)
	{
		...
		var dispatcherMethod = _dispatcherMethods.GetOrAdd(
			key: domainEventType, 
			valueFactory: _ => _dispatchToViewGenericMethod.MakeGenericMethod(domainEventType)
		);
		...
		{
			throw new ApplicationException($"Could not dispatch {domainEvent} to {view}", exception.InnerException);
		}
		catch (Exception exception)
		{
			throw new ApplicationException($"Could not dispatch {domainEvent} to {view}", exception);
		}
	}
	...
	void DispatchToViewGeneric<TDomainEvent>(
		IViewContext context, 
		TDomainEvent domainEvent, 
		IViewInstance viewInstance) 

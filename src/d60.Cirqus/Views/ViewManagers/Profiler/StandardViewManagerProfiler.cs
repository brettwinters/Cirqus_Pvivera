...
public class StandardViewManagerProfiler : IViewManagerProfiler
{
	...
	public ViewManagerStatsResult GetStats()
	{
		...
		foreach (var viewManager in viewManagers)
		{
			...
			if (!_timeSpent.TryRemove(viewManager, out trackings))
			{
				continue;
			}
	...
	public class ViewManagerStats : IEnumerable<DomainEventStats>
	{
		...
		{
			return $@"{ViewManager.GetType().GetPrettyName()}:
{string.Join(Environment.NewLine, this.Select(s => s.ToString()).Indented())}";
		}
	...
	public class DomainEventStats
	{
		...
		public override string ToString()
		{
			return $"{DomainEventType.Name}: {Elapsed.TotalSeconds:0.0} s ({NumberOfOccurrences})";
		}
	}
}
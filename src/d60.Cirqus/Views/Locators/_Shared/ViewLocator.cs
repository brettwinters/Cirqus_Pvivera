...
/// <summary>
/// Abstracts away the logic of determining the scope of view instances by mapping from a
/// <see cref="DomainEvent"/> to a view id
/// </summary>
public abstract class ViewLocator
{
	...
	/// </summary>
	public IEnumerable<string> GetAffectedViewIds(
	...
	/// <summary>
	/// Looks at the type closing the implemented <see cref="IViewInstance{TViewLocator}"/> and
	/// returns a new instance of that type
	/// </summary>
	static ViewLocator ActivateNewViewLocatorInstanceFromClosingType(Type viewType)
	{
		...
		if (genericViewType == null)
		{
			throw new ArgumentException(
				"Could not construct view dispatcher because the given" + 
				$" view type {viewType} is not an implementation of IViewInstance<> (i.e." + 
				" you must implement IViewInstance<TViewLocator> so that it can be" + 
				" figured out how to locate a specific view instance of" + 
				" this type for each domain event)"
			);
		}
		...
		{
			throw new ApplicationException($"Could not construct a view locator of type {viewLocatorType}", exception);
		}
	}
}
...
/// <summary>
/// Special view manager event dispatcher that can have other views as a
...
public class DependentViewManagerEventDispatcher : IDisposable, IAwaitableEventDispatcher
{
	...
	public int MaxDomainEventsPerBatch
	{
		...
		set
		{
			if (value < 1)
			{
				throw new ArgumentException($"Attempted to set MAX items per batch to {value}! Please set it to at least 1...");
			}

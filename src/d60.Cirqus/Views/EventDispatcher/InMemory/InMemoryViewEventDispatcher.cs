			...
			foreach (var e in events)
			{
				try
				{
					...
					foreach (var viewId in affectedViewIds)
					{
						...
						{
							throw new ApplicationException($"An error ocurred when dispatching {e} to view with ID {viewId}", exception);
						}
					}
				...
				{
					throw new ApplicationException($"Could not dispatch {e} to view(s)", exception);
				}
			}
		}
		catch (Exception exception)

...
public class CommandProcessingResult
{
	...
	protected CommandProcessingResult(
		long? newPosition)
	{
	...
	// public static CommandProcessingResult WithNewPosition(
	// 	long newPosition,
	// 	long timestamp)
	// {
	// 	var result = new CommandProcessingResult(newPosition) { TimeStamp = timestamp };
	// 	return result;
	// }
	...
	/// </summary>
	public bool EventsWereEmitted => _newPosition.HasValue;

	/// <summary>
	/// Returns the global sequence number of the last event that was emitted.
	/// If no events were emitted, the number is unknown and this method will throw an exception
	/// </summary>
	public long GetNewPosition()
	{
		if (!_newPosition.HasValue)
		{
			throw new InvalidOperationException("Cannot get new position from a command processing result when no " +
			                                    "events were emitted!");
		}
